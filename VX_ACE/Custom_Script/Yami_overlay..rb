...
module YSA
  module OVERLAY
    ...
    # Default: ON
    #=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
    LIGHT_SWITCH = 30        # Turn on/off light layer
    SHADOW_SWITCH = 31       # Turn on/off shadow layer
    PARALLAX_SWITCH = 32     # Turn on/off parallax layer
    GROUND_SWITCH = 33       # Turn on/off ground layer
